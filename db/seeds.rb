...
# Rewrite issue type based on manual input

CSV.foreach('/Users/stephanietong/Dropbox/DBC/phase3/codesores-backend/db/training.csv') do |row|
  fields = row.split(',')
  p "FIELDS: #{fields}"
  title = fields[0][0]
  input_type = fields[0][1].to_i
  p title
  p input_type
  begin
    issue = Issue.find_by(title: title)
    p issue
    issue.request_type_id = input_type
    issue.save!
    p "DONE!"
  rescue
    p "NO MATCH FOUND"
